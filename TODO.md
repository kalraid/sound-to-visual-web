...
- [ ] **D1. 성부=각자 독립 경로 (사용자 지목, 최우선)**: "화성끼리 같은 선에서 가는 게 아니라
      각각 따로따로 가야 한다." 레퍼런스는 각 성부가 **자기만의 가느다란 리본 길**을 따로 통과
      (평평한 병렬 레인 X, XZ 평면에서 굽이치는 얇은 경로). 현재 우리: 직선 병렬 레인 + 화음은
      최고음 한 줄로 축약(ADR 0003). → ① 성부별 경로를 시각적으로 더 분리(레인 간격/높이 차/색
      대비), ② 화음 성부는 동시음을 한 줄로 뭉개지 말고 별도 마커/경로로 표현하는 옵션 검토.
      ⚠️ C2 추격은 후행을 선행 '같은 레인'에 올리는데(=같은 선), 이 지적과 상충 가능 →
      "추격은 같은 트랙, 일반 성부는 분리"의 경계를 명확히 하거나 추격 표현 방식 재검토.
- [ ] **D2. 거울 반사축 교정 (C4 후속)**: 레퍼런스 거울상은 **바닥면 아래(y 반전)로 내린 검은 수면
      반사**다. 현재 C4는 **옆 레인으로** 반사 → 방향 틀림. `_buildMirror()`에서 inversion 기본을
      "아래로 내리는 거울상(y→-y, 바닥 밑 검은 고스트)"로 바꾸고, 같은 x·z에 겹쳐 수면 반사처럼.
      (No.13식 직교 3평면 반사는 후순위 스트레치.)
- [ ] **D3. 곡(악장) 단위 장면화 + 타이틀 카드 + 전환**: 카논마다 완전히 다른 독립 조형 + "No.N"
      라벨 + 카메라 프레이밍 전환. 현재 디오라마는 10초 균등분할. → 분할 기준을 마디/프레이즈/
      섹션 경계로, 섹션 라벨 오버레이, 섹션 호핑 카메라. (Phase B 개선 항목과 합류.)
- [ ] **D4. 굽이치는 얇은 리본 트랙(스트레치)**: 트랙이 높이만 변하는 직선이 아니라 XZ에서
      굽이치는 얇은 리본/계단/고리. 큰 변경 — 엔진의 경로 모델을 1D(시간) 직선에서 곡선 경로로
      확장해야 함. 후순위, 별도 ADR 필요.
