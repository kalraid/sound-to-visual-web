...
- [ ] **I6. 구조 비트 수렴 (가산 레이어)** — 다운비트/카덴스에서 트랙들이 한 점으로 당겨지는 수렴.
      Q5 옵션3. F1 measures/다운비트 기반. 선택 토글.
- [ ] **I7. 카메라·미감 튜닝** — 나선 전체가 보이며 교차가 읽히는 카메라. 회전 속도·반경·옥타브 높이
      스케일 조율. 광역 음역(옥타브 많음) 입력의 나선 높이 압축 처리. (현재 카메라 1차 구현됨)
