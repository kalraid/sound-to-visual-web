...
- [x] **G1. 섹션별 고유 3D 조형** ✅ (2026-06-24): `_computeSegStats()` — 섬별 음표 밀도·음역 계산. 밀도>0.65+음역>12→막대숲(5×3 기둥), 밀도>0.3→계단(4단), 나머지→슬래브. `_addSegPillars()`, `_addSegSteps()` 추가.
- [x] **G2. 직교 3평면 반사(No.13 스타일)** ✅ (2026-06-24): `_buildCornerReflect()` — 캔버스 텍스처 3장(뒤벽=시간×음높이, 바닥=시간×레인, 옆벽=레인×음높이). scroll 모드 전용, 토글 "3면반사" 추가. `cornerGroup` + `setCornerReflect()` 신설.
- [x] **G3. 주제 타이틀 오프닝** ✅ (2026-06-24): `buildThemeOpening()` — 주성부 첫 8음을 A~H 라벨 큐브 한 줄로 X<0 위치에 배치. 분석 직후 카메라 정면 포지셔닝. `clearThemeOpening()` — 재생 버튼 클릭 시 제거. matte/glow 재질 대응.

## Phase I — 롤러코스터 모드: 공유 피치 나선 + 음 교차  ⬅ 신규 주력 방향
> 근거: ADR 0015, grill-with-docs 세션(2026-06-24). GOAL.md의 디오라마 재현과 **동등한 두 모드**로
> 둔다(같은 엔진, 토글). 핵심: "한 트랙 공유"가 아니라 **여러 트랙이 공통 음 위치에서 교차**.
> CONTEXT.md 신규 용어: 롤러코스터 모드 / 피치 나선 / 교차점 / lap / 코드 진행 주기.

- [x] **I1. 주행경로 토글 (v0, 임시)** ✅ (2026-06-24): `path-select`(직선|롤러코스터). v0 장식
      나선이었으나 I2에서 피치 나선 모델로 교체됨. 토글/레일 인프라(`_buildCoasterRails`)는 유지.
- [x] **I2. 공유 피치 나선 좌표계** ✅ (2026-06-24): `_helixPos(midi)` — θ=midi/12·2π, y∝midi.
      같은 음=같은 점 → 교차. `_pitchAtTime`(음 지속 유지+공백 보간)으로 시간=활공 매개변수.
      성부별 레일은 첫 음에서 출발(제각각), 공통 음에서 교차. coaster 시 디오라마/거울/코너/평면지형
      숨김, 카메라는 나선 부감. 검증: 같은음=같은점·옥타브=같은스포크, 실데이터에서 두 성부 실제 교차.
- [x] **I3. 전역 회전 = lap** ✅ (2026-06-24): `_computeLap()` — 코드 진행 주기(I4 harmony.detected)
      → 마디(4마디=1바퀴) → 고정 8초, [3,16]초 클램프. `helixRot = position/lapSec·2π`, 재생 위치가
      구동. coasterGroup.rotation.y + 큐브 좌표 동일 회전(상대 교차 보존). 검증: 1 lap=1바퀴.
- [x] **I4. 백엔드 화성 진행 분석 (option 2)** ✅ (2026-06-24): `classifier.harmony_detail()` —
      등간격 시간창 × 베이스 가중 12차원 피치클래스 벡터열 → 자기 유사도 최대 반복 주기 추출.
      템플릿 매칭 없음. `analysis.harmony{detected, progressionPeriodSec, confidence, windowSec}` 노출.
      검증: pachelbel 0.94/8s 감지, 실 캐논변주 ~0.76, melody 0.31. 테스트 6종 통과.
      ⚠️ Canon-Rock(0.56)은 실현 변주가 커 미감지 → lap은 마디 폴백.
- [x] **I5. 교차 강조** ✅ (2026-06-24): `_detectCrossings()` — 두 성부가 같은 음(반음 미만, 같은
      나선 점)에 만나는 '진입 순간'에만 흰 스파크 + 양쪽 큐브 플래시. 옥타브차 제외. `_crossState`로
      쌍 근접 상태 추적(스팸 방지), load 시 초기화. 검증: Oooo!(16성부) 교차 275프레임, 120쌍.
- [x] **I6. 구조 비트 수렴 (가산 레이어)** ✅ (2026-06-25): `convergeOn`/`convergence` 상태 추가. 마디 경계(`measures`) 통과 시 convergence=1.0 → dt*2.2 감쇠. 코스터 모드에서 큐브를 나선 중심축으로 lerp pull(strength=0.72). `setConverge()` + `converge-select` 토글 추가.
- [x] **I7. 카메라·미감 튜닝** ✅ (2026-06-25): `_helixOctH()` — 음역 폭에 따라 옥타브당 높이 압축(최대 높이 20 단위 클램프). 카메라: helixRot×0.25 위상 오프셋으로 서서히 나선 주위 공전, 나선 높이에 비례한 수직 위치 조정.
