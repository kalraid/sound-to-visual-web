...
- [~] **I1. 주행경로 토글 (v0, 임시)** — `path-select`(직선|롤러코스터) + 성부별 장식 나선·튜브 레일.
      구현·검증됨(2026-06-24)이나 **ADR 0015 모델과 다름**(위상만 다른 독립 나선, 교차 의미 없음).
      I2에서 피치 나선 + 교차 모델로 교체 예정. `terrain.js` `_coasterPos`/`_buildCoasterRails`.
- [ ] **I2. 공유 피치 나선 좌표계** — 음 → (피치클래스=각도, 옥타브=높이)로 매핑하는 공유 기하.
      시간은 공간축이 아니라 활공 매개변수(`_helixPos(midi)` + 음 사이 보간). 거울/코너/디오라마는
      이 모드에서 숨김. `_worldXZ`와 별개 좌표 변환 레이어.
- [ ] **I3. 전역 회전 = lap** — 나선 전체가 회전, 1바퀴 = 1 코드 진행 주기(I4 period). period 없으면
      마디/다운비트(F1 measures), 그것도 없으면 고정 기본값. 회전은 상대 교차를 보존(전 성부 동일 회전).
- [ ] **I4. 백엔드 화성 진행 분석 (option 2)** — 템플릿 없이 **반복 코드 주기(period) 감지**.
      music21 chordify → 윈도(박/마디) 코드열 → 자기 유사도로 순환 길이 추출.
      `analysis.harmony: {progressionPeriodSec, detected, ...}` 노출. lap 구동 + 솔로 화성-진행
      캐논(Canon-Rock류) 분류 보완. ⚠️ 파헬벨 특정 템플릿 매칭은 채택 안 함(ADR 0015).
- [ ] **I5. 교차 강조** — 두 성부가 공통 음(같은 나선 점)에 수렴하는 순간 시각 강조(플래시/스파크).
      기본 레이어. 캐논은 후행이 선행 경로를 lagSec 지연 추격 → 반복 교차로 캐논 본질이 드러남.
...
- [ ] **I7. 카메라·미감 튜닝** — 나선 전체가 보이며 교차가 읽히는 카메라. 회전 속도·반경·옥타브 높이
      스케일 조율. 광역 음역(옥타브 많음) 입력의 나선 높이 압축 처리.
